  ...
  const handleImport = async () => {
    if (!selectedGroupId) {
      toast({
        ...
        description: "Choose which group to add the contacts to",
        variant: "destructive",
      });
      return;
    }
    ...
    setIsLoading(true);
    try {
      const response = await apiRequest(`/api/projects/${projectId}/contacts/bulk-import`, {
        method: "POST",
        body: JSON.stringify({
            ...
            <div>
              ...
              <div className="mt-2 max-h-64 overflow-y-auto border border-gray-200 rounded">
                <div className="text-xs bg-gray-50 sticky top-0">
                  <div className="grid grid-cols-4 gap-2 p-2 font-medium border-b">
                    <div>First Name</div>
                    <div>Last Name</div>
                    <div>Email</div>
                    <div>Role</div>
                ...
                <div className="text-xs">
                  {previewData.slice(0, 10).map((contact, idx) => (
                    <div key={idx} className="grid grid-cols-4 gap-2 p-2 border-b hover:bg-gray-50">
