...
export default function CompanyList() {
  ...
  const [headerText, setHeaderText] = useState('{{showName}} - Company List');
  ...
  const debouncedAutoSave = useCallback(() => {
    ...
    saveTimeoutRef.current = setTimeout(async () => {
      ...
      try {
        ...
        try {
          // Save to database
          await apiRequest('POST', '/api/projects/' + projectId + '/company-list-settings', contactSheetSettings);
        } catch (dbError) {
          console.warn('Database save failed, using localStorage fallback:', dbError);
        }
        
        // Always save to localStorage as backup
        localStorage.setItem(`company-list-settings-${projectId}`, JSON.stringify(contactSheetSettings));
        
        setLastSaved(new Date());
      } catch (error) {
        console.error('Failed to auto-save company list settings:', error);
      } finally {
        setIsSaving(false);
      }
  ...
  // Load saved company list settings
  const { data: savedSettings } = useQuery({
    queryKey: [`/api/projects/${projectId}/company-list-settings`],
    enabled: !!projectId,
  });

  // Load current version
  const { data: versionData } = useQuery({
    queryKey: [`/api/projects/${projectId}/company-list/current-version`],
    enabled: !!projectId,
  });
  ...
  const publishVersionMutation = useMutation({
    mutationFn: async ({ versionType, settings }: { versionType: 'major' | 'minor', settings: any }) => {
      const response = await fetch(`/api/projects/${projectId}/company-list/publish`, {
        method: 'POST',
    ...
    onSuccess: (data: any) => {
      ...
      queryClient.invalidateQueries({ queryKey: [`/api/projects/${projectId}/company-list/current-version`] });
      queryClient.refetchQueries({ queryKey: [`/api/projects/${projectId}/company-list/current-version`] });
      toast({
        title: `${data.versionType === 'major' ? 'Major' : 'Minor'} Version Published Successfully`,
        description: `Company list version ${data.version} has been published.`,
      });
    },
    onError: (error: any) => {
      setIsPublishing(false);
  ...
  useEffect(() => {
    ...
    const localSettings = localStorage.getItem(`company-list-settings-${projectId}`);
    ...
    if (settings) {
      ...
      setHeaderText(settings.headerText || '{{showName}} - Company List');
    ...
    <div className="min-h-screen bg-background">
      <div className="container mx-auto p-6 print:hidden">
        ...
        <div className="mb-8 flex justify-between items-start">
          <div>
            <h1 className="text-3xl font-bold">Company List - {(project as any)?.name}</h1>
      ...
      {!isPreviewMode && (
        <div className="fixed right-4 top-1/2 transform translate-y-56 bg-white border rounded-lg shadow-lg p-4 print:hidden">
          <h4 className="font-semibold mb-3 flex items-center gap-2">
