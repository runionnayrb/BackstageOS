...
- **V2 Report Template System - Core Complete**: Implemented production-ready template system with normalized data model (report_templates_v2 → template_sections → template_fields). Backend features composite indexes for optimal query performance, parallel data loading via single batched endpoint, transactional reordering, and comprehensive Zod validation. Frontend includes template list page with clickable cards and optimistic CRUD updates, full-featured template editor with section/field management and department assignment, and departments management tab in Show Settings. System uses form-based approach (not WYSIWYG) optimized for speed with parallel loading and client-side optimistic updates. Access via `/shows/:id/templates-v2` route.
