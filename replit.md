...
- June 28, 2025: **DRAG-AND-DROP SPACING CONTROLS**: Implemented comprehensive drag-and-drop spacing adjustment system for contact sheets with visual handles. Added category spacing control (purple handle) between section titles and table headers, and section spacing control (blue handle) between different contact categories. Fixed resize logic to allow spacing values down to 0px while maintaining 20px minimum for header/row heights. Users can now create very tight layouts by dragging spacing handles upward to compress space between elements.
