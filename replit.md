...
- July 2, 2025: **SHOW SCHEDULE FILTERING SYSTEM WITH EVENT TYPE CATEGORIZATION**: Implemented comprehensive "Show Schedule" functionality replacing "Master Schedule" terminology. Created event type categorization system distinguishing show-wide events (rehearsal, tech, preview, performance, talkback) from individual events (meeting, other). Show Schedule now displays only production-wide events that everyone participates in when no contact filter is applied. Added new event types (preview, talkback) and centralized event type management through eventUtils.ts utility with display names, colors, and categorization logic. Updated all three schedule views (Monthly, Weekly, Daily) to filter events based on type classification, ensuring clean separation between show-wide production events and individual appointments.
