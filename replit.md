...
- **Document Templating System**: Implemented Plumsail-style document templating allowing users to upload custom Word (.docx) templates with {{variable}} placeholders. Templates integrate with export workflows - when users export Reports or Contacts, the system uses their custom template if one exists, otherwise falls back to default PDF formatting. Users can browse available variables (organized by document type: report, daily_call, contacts, running_order, cast_list, crew_list, schedule) with copy-to-clipboard functionality in Show Settings > Documents tab. Backend uses docxtemplater and pizzip for Word document processing.
