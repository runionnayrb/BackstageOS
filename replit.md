...
- June 28, 2025: **IMPROVED IMPORT SCRIPT FUNCTIONALITY**: Enhanced script import with comprehensive file validation to prevent binary file errors. Added support for text files (.txt), RTF files (.rtf), and HTML files (.html) with proper file type validation, size limits (10MB), binary content detection, and user-friendly error messages. Prevents PDF and other binary files from causing database errors while providing clear guidance on supported formats.
