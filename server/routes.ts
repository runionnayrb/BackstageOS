...
-Stage Management`;
  ...
  // Lightweight schedule header endpoint - returns only metadata needed for header controls
  app.get('/api/projects/:projectId/schedule-header', isAuthenticated, async (req: any, res) => {
    try {
      const projectId = parseInt(req.params.projectId);
      ...
      // Get versions for this week only (lightweight query)
      const allVersions = await storage.getScheduleVersionsByProjectId(projectId);
      const weekVersions = weekStart 
        ? allVersions.filter((v: any) => v.weekStart === weekStart)
        : [];
      
      // Get the current published version for this week
      let currentVersion = null;
      if (weekVersions.length > 0) {
        const sortedVersions = [...weekVersions].sort((a: any, b: any) => 
          new Date(b.publishedAt).getTime() - new Date(a.publishedAt).getTime()
        );
        currentVersion = {
          id: sortedVersions[0].id,
          version: sortedVersions[0].version,
          minorVersion: sortedVersions[0].minorVersion,
          publishedAt: sortedVersions[0].publishedAt,
          weekStart: sortedVersions[0].weekStart
        };
      }
      
      // Get personal schedules with minimal data for dropdown
      const personalSchedules = await storage.getPersonalSchedulesByProjectId(projectId);
      const organizedSchedules: Record<string, any[]> = {};
      
      for (const schedule of personalSchedules) {
        if (schedule.contact) {
          const groupName = (schedule.contact as any).contactGroup?.name || (schedule.contact as any).category || 'Other';
          const typeName = groupName.charAt(0).toUpperCase() + groupName.slice(1).replace(/_/g, ' ');
          
          if (!organizedSchedules[typeName]) {
            organizedSchedules[typeName] = [];
          }
          organizedSchedules[typeName].push({
            name: `${(schedule.contact as any).firstName || ''} ${(schedule.contact as any).lastName || ''}`.trim() || (schedule.contact as any).email || 'Unknown Contact',
            token: schedule.accessToken,
            contactId: (schedule.contact as any).id
          });
        }
      }
      ...
      res.json({
        ...
        personalSchedules: organizedSchedules,
        hasPersonalSchedules: Object.keys(organizedSchedules).length > 0
      });
    } catch (error) {
      console.error('Error fetching schedule header:', error);
      res.status(500).json({ message: "Failed to fetch schedule header" });
    }
  });
